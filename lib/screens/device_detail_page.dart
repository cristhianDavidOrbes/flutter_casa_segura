...
import 'package:flutter_seguridad_en_casa/screens/motion_threshold_page.dart';
...
class _DeviceDetailPageState extends State<DeviceDetailPage> {
  ...
  Widget? _buildDetectorSummary(ColorScheme cs) {
    ...
    final bool isDetector =
        ...
        data.containsKey('ultra_cm');
    if (!isDetector) return null;

    final ultrasonic = _stringKeyedMap(data['ultrasonic']);
    final double? distance = _asDouble(
      ultrasonic?['cm'] ?? data['ultra_cm'] ?? data['distance_cm'],
    );
    final bool? ultraOk = _asBool(
      ultrasonic?['ok'] ?? data['ultra_ok'] ?? data['ultrasonic_ok'],
    );

    final theme = Theme.of(context);
    final threshold =
        MotionSettingsService.instance.thresholdFor(widget.deviceId);

    final entries = <Widget>[
      _kv('Umbral de alerta', '${threshold.toStringAsFixed(0)} cm'),
    ];
    ...
    if (distance != null && distance >= 0) {
      ...
      entries.add(_kv('Distancia actual', '$formatted cm'));
    } else if (distance != null) {
      entries.add(_kv('Distancia actual', 'Sin lectura (eco no detectado)'));
    } else {
      entries.add(_kv('Distancia actual', 'Sin lectura'));
    }
    ...
    entries.add(
      Padding(
        ...
        child: Text(
          'Las lecturas negativas del sensor se ignoran automáticamente.',
      ...
      Padding(
        ...
        child: Align(
          ...
          child: TextButton.icon(
            onPressed: () async {
              final updated = await Navigator.of(context).push<double>(
                MaterialPageRoute(
                  builder: (_) => MotionThresholdPage(
                    deviceId: widget.deviceId,
            ...
            label: const Text('Ajustar sensibilidad'),
          ),
        ),
      ),
    );
