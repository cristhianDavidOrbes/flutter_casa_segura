...
class _ProvisioningScreenState extends State<ProvisioningScreen> {
  ...
  String _resolveSuccessMessage(ProvisioningResult result) {
    final message = result.message?.trim();
    if (message == null || message.isEmpty) {
      return 'Credenciales enviadas. El equipo se est! conectando a tu Wi-Fi...';
    }
    final lower = message.toLowerCase();
    if (lower.contains('abort') ||
        lower.contains('ap closed') ||
        lower.contains('ap_closed')) {
      return 'Credenciales enviadas. El equipo se reiniciar! y saldr! del modo AP.';
    }
  ...
  Future<void> _sendProvision() async {
    ...
    if (_apGoneAfterProvision) {
      Get.snackbar(
        'Provisioning',
        'El dispositivo cerró su AP y se est! conectando a tu Wi-Fi. Revisa la lista en unos segundos.',
        snackPosition: SnackPosition.BOTTOM,
        duration: const Duration(seconds: 4),
      );
      Get.off(() => const DevicesPage());
