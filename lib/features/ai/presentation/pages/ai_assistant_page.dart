...
class _AiAssistantPageState extends State<AiAssistantPage>
  ...
  Future<void> _handleSubmit(String raw) async {
    ...
    if (!_isSecurityRelated(text)) {
      ...
      final warning = 'ai.warning.outOfScope'.tr;
  ...
  void _evaluateCommand(String text) {
    ...
    final wantsVideo =
        ...
        lower.contains('ver la camara') ||

        lower.contains('ver en vivo') ||

        lower.contains('mostrar camara') ||

        lower.contains('muestra video') ||

        lower.contains('muestra la camara') ||

        lower.contains('show me the video') ||

        lower.contains('show the video') ||

        lower.contains('show live video') ||

        lower.contains('show live feed') ||

        lower.contains('show the live camera') ||

        lower.contains('live feed') ||

        lower.contains('live stream') ||

        lower.contains('live video');
    ...
    final removeVideo =
        ...
        lower.contains('oculta la camara') ||

        lower.contains('detener video') ||

        lower.contains('detener transmision') ||

        lower.contains('hide the video') ||

        lower.contains('close the video') ||

        lower.contains('stop the video') ||

        lower.contains('stop live') ||

        lower.contains('close live') ||

        lower.contains('hide live');
    ...
    if (wantsVideo) {
      ...
      unawaited(_appendAssistant('ai.local.videoEnabled'.tr));

    } else if (removeVideo) {

      _hideVideo();

      unawaited(_appendAssistant('ai.local.videoDisabled'.tr));
    ...
    if (wantsDetection) {
      ...
      unawaited(_appendAssistant('ai.local.detectionsEnabled'.tr));
      ...
      _detectionAnimCtrl.reverse().then((_) {
        ...
        unawaited(_appendAssistant('ai.local.detectionsDisabled'.tr));
    ...
    if (feed == null) {
      ...
      setState(() {
        ...
        _cameraError =
            ...
            'No hay un stream disponible para tus cmaras en este momento.';
  ...
  Future<bool> _handleLocalCommand(String text) async {
    ...
    if (lower.contains('conversacion') || lower.contains('conversation')) {
      ...
      await _appendAssistant('ai.history.opened'.tr);
  ...
  String _generateDailySummary() {
    ...
    if (events.length > 4) {
      ...
      buffer.writeln(
        ...
        '… ${events.length - 4} eventos adicionales.',
  ...
  String _listRecentNotifications() {
    ...
    buffer.writeln(
      ...
      'Puedes abrir la bandeja de notificaciones para ver las im!genes adjuntas.',
    ...
    if (feed == null) {
      ...
      return Container(
        ...
        child: Text(
          ...
          error ?? 'No fue posible abrir el stream de la cmara.',
